...
from apps.listing.models import Booking, GuestHouseBooking, PropertyRentalBooking
...
@receiver(post_save, sender=PropertyRentalBooking)
def schedule_property_rental_auto_cancel(sender, instance, created, **kwargs):
    if created and instance.status == PropertyRentalBooking.RentStatus.PENDING:
        timeout_minutes = getattr(settings, 'BOOKING_PENDING_TIMEOUT_MINUTES', 15)

        transaction.on_commit(
            lambda: auto_cancel_pending_property_rental_booking.apply_async(
                args=[instance.id],
                countdown=60 * timeout_minutes
            )
        )
...
@receiver(post_save, sender="listing.PropertyListing")
def auto_create_property_rental_availability(sender, instance, created, **kwargs):
    if created:
        from apps.listing.services import PropertyRentalAvailabilityService
        transaction.on_commit(
            lambda: PropertyRentalAvailabilityService.create_availability(instance)
        )
