# Generated by Django 5.2.4 on 2025-09-08 11:00

import django.db.models.deletion
import uuid
from django.conf import settings
from django.db import migrations, models


class Migration(migrations.Migration):
    initial = True

    dependencies = [
        ("account", "0001_initial"),
        ("contenttypes", "0002_remove_content_type_name"),
        ("core", "0001_initial"),
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name="Amenity",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4,
                        editable=False,
                        primary_key=True,
                        serialize=False,
                        verbose_name="Id",
                    ),
                ),
                (
                    "created_at",
                    models.DateTimeField(auto_now_add=True, verbose_name="Created At"),
                ),
                (
                    "updated_at",
                    models.DateTimeField(auto_now=True, verbose_name="Updated_at"),
                ),
                (
                    "name",
                    models.CharField(max_length=255, unique=True, verbose_name="Name"),
                ),
                (
                    "icon",
                    models.CharField(blank=True, max_length=100, verbose_name="Icon"),
                ),
            ],
            options={
                "verbose_name": "Amenity",
                "verbose_name_plural": "Amenities",
                "db_table": "amenities",
            },
        ),
        migrations.CreateModel(
            name="EventSpaceListing",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4,
                        editable=False,
                        primary_key=True,
                        serialize=False,
                        verbose_name="Id",
                    ),
                ),
                (
                    "created_at",
                    models.DateTimeField(auto_now_add=True, verbose_name="Created At"),
                ),
                (
                    "updated_at",
                    models.DateTimeField(auto_now=True, verbose_name="Updated_at"),
                ),
                (
                    "title",
                    models.CharField(
                        help_text="Title of the listing, e.g., 'Luxury Suite with Pool View'.",
                        max_length=255,
                        verbose_name="Title",
                    ),
                ),
                (
                    "description",
                    models.TextField(
                        blank=True,
                        help_text="Detailed description of the listing.",
                        verbose_name="Description",
                    ),
                ),
                (
                    "base_price",
                    models.DecimalField(
                        decimal_places=2,
                        help_text="Price of the service/product (if applicable).",
                        max_digits=10,
                        verbose_name="Price",
                    ),
                ),
                (
                    "is_active",
                    models.BooleanField(
                        default=True,
                        help_text="Whether the listing is active and visible.",
                        verbose_name="Is Active",
                    ),
                ),
                ("number_of_guests", models.PositiveIntegerField(default=1)),
                (
                    "space_type",
                    models.CharField(
                        choices=[
                            ("auditorium", "Auditorium"),
                            ("conference_hall", "Conference Hall"),
                            ("meeting_room", "Meeting Room"),
                        ],
                        max_length=50,
                    ),
                ),
                ("floor_area_sqm", models.PositiveIntegerField(blank=True, null=True)),
                (
                    "address",
                    models.OneToOneField(
                        blank=True,
                        on_delete=django.db.models.deletion.RESTRICT,
                        related_name="+",
                        to="core.address",
                    ),
                ),
                (
                    "amenities",
                    models.ManyToManyField(
                        blank=True,
                        related_name="event_space_listings",
                        to="listing.amenity",
                        verbose_name="Amenities",
                    ),
                ),
                (
                    "hotel",
                    models.ForeignKey(
                        blank=True,
                        help_text="The company that owns this listing.",
                        null=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="event_space_listings",
                        to="account.hotelprofile",
                        verbose_name="Company",
                    ),
                ),
            ],
            options={
                "verbose_name": "Event space",
                "verbose_name_plural": "Event Spaces",
                "db_table": "event_spaces",
            },
        ),
        migrations.CreateModel(
            name="Booking",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4,
                        editable=False,
                        primary_key=True,
                        serialize=False,
                        verbose_name="Id",
                    ),
                ),
                (
                    "created_at",
                    models.DateTimeField(auto_now_add=True, verbose_name="Created At"),
                ),
                (
                    "updated_at",
                    models.DateTimeField(auto_now=True, verbose_name="Updated_at"),
                ),
                ("units_booked", models.PositiveIntegerField(default=1)),
                ("check_in_date", models.DateField()),
                ("check_out_date", models.DateField()),
                ("total_price", models.DecimalField(decimal_places=2, max_digits=10)),
                (
                    "status",
                    models.CharField(
                        choices=[
                            ("pending", "Pending"),
                            ("confirmed", "Confirmed"),
                            ("cancelled", "Cancelled"),
                        ],
                        default="pending",
                        max_length=20,
                    ),
                ),
                (
                    "user",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        to=settings.AUTH_USER_MODEL,
                    ),
                ),
                (
                    "event_space",
                    models.ForeignKey(
                        blank=True,
                        null=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="bookings",
                        to="listing.eventspacelisting",
                    ),
                ),
            ],
            options={
                "verbose_name": "Booking",
                "verbose_name_plural": "Bookings",
                "db_table": "bookings",
            },
        ),
        migrations.CreateModel(
            name="ListingImage",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4,
                        editable=False,
                        primary_key=True,
                        serialize=False,
                        verbose_name="Id",
                    ),
                ),
                (
                    "created_at",
                    models.DateTimeField(auto_now_add=True, verbose_name="Created At"),
                ),
                (
                    "updated_at",
                    models.DateTimeField(auto_now=True, verbose_name="Updated_at"),
                ),
                ("object_id", models.UUIDField(verbose_name="Object ID")),
                (
                    "image",
                    models.ImageField(
                        help_text="Upload an image for the listing.",
                        upload_to="listing_images/",
                        verbose_name="Image File",
                    ),
                ),
                (
                    "alt_text",
                    models.CharField(
                        blank=True,
                        help_text="Alternative text for the image.",
                        max_length=255,
                        verbose_name="Alt Text",
                    ),
                ),
                (
                    "is_primary",
                    models.BooleanField(
                        default=False,
                        help_text="Marks the main image for the listing.",
                        verbose_name="Primary Image",
                    ),
                ),
                (
                    "content_type",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        to="contenttypes.contenttype",
                        verbose_name="Content Type",
                    ),
                ),
            ],
            options={
                "verbose_name": "Listing Image",
                "verbose_name_plural": "Listing Images",
                "db_table": "listing_images",
            },
        ),
        migrations.AddField(
            model_name="eventspacelisting",
            name="images",
            field=models.ManyToManyField(to="listing.listingimage"),
        ),
        migrations.CreateModel(
            name="Payment",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4,
                        editable=False,
                        primary_key=True,
                        serialize=False,
                        verbose_name="Id",
                    ),
                ),
                (
                    "created_at",
                    models.DateTimeField(auto_now_add=True, verbose_name="Created At"),
                ),
                (
                    "updated_at",
                    models.DateTimeField(auto_now=True, verbose_name="Updated_at"),
                ),
                ("amount", models.DecimalField(decimal_places=2, max_digits=10)),
                (
                    "status",
                    models.CharField(
                        choices=[
                            ("pending", "Pending"),
                            ("paid", "Paid"),
                            ("failed", "Failed"),
                            ("refund", "Refund"),
                        ],
                        default="pending",
                        max_length=20,
                    ),
                ),
                (
                    "transaction_id",
                    models.CharField(blank=True, max_length=100, null=True),
                ),
                (
                    "booking",
                    models.OneToOneField(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="payment",
                        to="listing.booking",
                    ),
                ),
            ],
            options={
                "verbose_name": "Payment",
                "verbose_name_plural": "Payments",
                "db_table": "payments",
            },
        ),
        migrations.CreateModel(
            name="RoomListing",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4,
                        editable=False,
                        primary_key=True,
                        serialize=False,
                        verbose_name="Id",
                    ),
                ),
                (
                    "created_at",
                    models.DateTimeField(auto_now_add=True, verbose_name="Created At"),
                ),
                (
                    "updated_at",
                    models.DateTimeField(auto_now=True, verbose_name="Updated_at"),
                ),
                (
                    "title",
                    models.CharField(
                        help_text="Title of the listing, e.g., 'Luxury Suite with Pool View'.",
                        max_length=255,
                        verbose_name="Title",
                    ),
                ),
                (
                    "description",
                    models.TextField(
                        blank=True,
                        help_text="Detailed description of the listing.",
                        verbose_name="Description",
                    ),
                ),
                (
                    "base_price",
                    models.DecimalField(
                        decimal_places=2,
                        help_text="Price of the service/product (if applicable).",
                        max_digits=10,
                        verbose_name="Price",
                    ),
                ),
                (
                    "is_active",
                    models.BooleanField(
                        default=True,
                        help_text="Whether the listing is active and visible.",
                        verbose_name="Is Active",
                    ),
                ),
                ("number_of_guests", models.PositiveIntegerField(default=1)),
                (
                    "total_units",
                    models.PositiveIntegerField(
                        help_text="How many rooms of this type exist in the property.",
                        verbose_name="Total Rooms of This Type",
                    ),
                ),
                (
                    "bed_type",
                    models.CharField(
                        choices=[
                            ("king", "King"),
                            ("queen", "Queen"),
                            ("twin", "Twin"),
                            ("double", "Double"),
                            ("mixed", "Mixed/Multiple"),
                        ],
                        default="mixed",
                        max_length=20,
                    ),
                ),
                ("room_size_sqm", models.PositiveIntegerField()),
                ("smoking_allowed", models.BooleanField(default=False)),
                ("children_allowed", models.BooleanField(default=True)),
                (
                    "refundable",
                    models.BooleanField(
                        default=True, help_text="Whether standard rate is refundable."
                    ),
                ),
                (
                    "address",
                    models.OneToOneField(
                        blank=True,
                        on_delete=django.db.models.deletion.RESTRICT,
                        related_name="+",
                        to="core.address",
                    ),
                ),
                (
                    "amenities",
                    models.ManyToManyField(
                        blank=True,
                        related_name="room_listings",
                        to="listing.amenity",
                        verbose_name="Amenities",
                    ),
                ),
                (
                    "hotel",
                    models.ForeignKey(
                        blank=True,
                        help_text="The company that owns this listing.",
                        null=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="room_listings",
                        to="account.hotelprofile",
                        verbose_name="Company",
                    ),
                ),
                ("images", models.ManyToManyField(to="listing.listingimage")),
            ],
            options={
                "verbose_name": "Room Type",
                "verbose_name_plural": "Room Types",
                "db_table": "room_listings",
            },
        ),
        migrations.AddField(
            model_name="booking",
            name="room",
            field=models.ForeignKey(
                blank=True,
                null=True,
                on_delete=django.db.models.deletion.CASCADE,
                related_name="bookings",
                to="listing.roomlisting",
            ),
        ),
        migrations.CreateModel(
            name="EventSpaceAvailability",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4,
                        editable=False,
                        primary_key=True,
                        serialize=False,
                        verbose_name="Id",
                    ),
                ),
                (
                    "created_at",
                    models.DateTimeField(auto_now_add=True, verbose_name="Created At"),
                ),
                (
                    "updated_at",
                    models.DateTimeField(auto_now=True, verbose_name="Updated_at"),
                ),
                ("date", models.DateField(db_index=True)),
                (
                    "price",
                    models.DecimalField(
                        blank=True, decimal_places=2, max_digits=10, null=True
                    ),
                ),
                (
                    "space_listing",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="availability",
                        to="listing.eventspacelisting",
                    ),
                ),
            ],
            options={
                "verbose_name": "Event space Availability",
                "verbose_name_plural": "Event Space Availabilities",
                "db_table": "event_space_availabilities",
                "ordering": ["date"],
                "unique_together": {("space_listing", "date")},
            },
        ),
        migrations.CreateModel(
            name="GuestHouseListing",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4,
                        editable=False,
                        primary_key=True,
                        serialize=False,
                        verbose_name="Id",
                    ),
                ),
                (
                    "created_at",
                    models.DateTimeField(auto_now_add=True, verbose_name="Created At"),
                ),
                (
                    "updated_at",
                    models.DateTimeField(auto_now=True, verbose_name="Updated_at"),
                ),
                (
                    "title",
                    models.CharField(
                        help_text="Title of the listing, e.g., 'Luxury Suite with Pool View'.",
                        max_length=255,
                        verbose_name="Title",
                    ),
                ),
                (
                    "description",
                    models.TextField(
                        blank=True,
                        help_text="Detailed description of the listing.",
                        verbose_name="Description",
                    ),
                ),
                (
                    "base_price",
                    models.DecimalField(
                        decimal_places=2,
                        help_text="Price of the service/product (if applicable).",
                        max_digits=10,
                        verbose_name="Price",
                    ),
                ),
                (
                    "is_active",
                    models.BooleanField(
                        default=True,
                        help_text="Whether the listing is active and visible.",
                        verbose_name="Is Active",
                    ),
                ),
                (
                    "total_rooms",
                    models.PositiveIntegerField(
                        help_text="Number of rooms available in the guest house.",
                        verbose_name="Total Rooms",
                    ),
                ),
                (
                    "rating",
                    models.DecimalField(
                        blank=True,
                        decimal_places=2,
                        help_text="Average rating from user reviews (0–5).",
                        max_digits=3,
                        null=True,
                        verbose_name="Average Rating",
                    ),
                ),
                (
                    "address",
                    models.OneToOneField(
                        blank=True,
                        on_delete=django.db.models.deletion.RESTRICT,
                        related_name="+",
                        to="core.address",
                    ),
                ),
                (
                    "amenities",
                    models.ManyToManyField(
                        blank=True,
                        related_name="guest_house_listings",
                        to="listing.amenity",
                        verbose_name="Amenities",
                    ),
                ),
                (
                    "company",
                    models.ForeignKey(
                        blank=True,
                        help_text="The company that owns this listing.",
                        null=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="guest_house_listings",
                        to="account.companyprofile",
                        verbose_name="Company",
                    ),
                ),
                (
                    "individual_owner",
                    models.ForeignKey(
                        blank=True,
                        help_text="The individual person that owns this listing.",
                        null=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="guest_house_listings",
                        to="account.individualownerprofile",
                        verbose_name="Individual Owner",
                    ),
                ),
                ("images", models.ManyToManyField(to="listing.listingimage")),
            ],
            options={
                "verbose_name": "Guest House",
                "verbose_name_plural": "Guest Houses",
                "db_table": "guest_houses",
                "constraints": [
                    models.CheckConstraint(
                        condition=models.Q(
                            models.Q(
                                ("individual_owner__isnull", False),
                                ("company__isnull", True),
                            ),
                            models.Q(
                                ("individual_owner__isnull", True),
                                ("company__isnull", False),
                            ),
                            _connector="OR",
                        ),
                        name="guest_house_owner_must_exist",
                    )
                ],
            },
        ),
        migrations.CreateModel(
            name="CarListing",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4,
                        editable=False,
                        primary_key=True,
                        serialize=False,
                        verbose_name="Id",
                    ),
                ),
                (
                    "created_at",
                    models.DateTimeField(auto_now_add=True, verbose_name="Created At"),
                ),
                (
                    "updated_at",
                    models.DateTimeField(auto_now=True, verbose_name="Updated_at"),
                ),
                (
                    "title",
                    models.CharField(
                        help_text="Title of the listing, e.g., 'Luxury Suite with Pool View'.",
                        max_length=255,
                        verbose_name="Title",
                    ),
                ),
                (
                    "description",
                    models.TextField(
                        blank=True,
                        help_text="Detailed description of the listing.",
                        verbose_name="Description",
                    ),
                ),
                (
                    "base_price",
                    models.DecimalField(
                        decimal_places=2,
                        help_text="Price of the service/product (if applicable).",
                        max_digits=10,
                        verbose_name="Price",
                    ),
                ),
                (
                    "is_active",
                    models.BooleanField(
                        default=True,
                        help_text="Whether the listing is active and visible.",
                        verbose_name="Is Active",
                    ),
                ),
                (
                    "brand",
                    models.CharField(
                        choices=[
                            ("toyota", "Toyota"),
                            ("bmw", "BMW"),
                            ("Mercedes-Benz", "Mercedes-Benz"),
                            ("audi", "Audi"),
                            ("Volkswagen", "Volkswagen"),
                            ("honda", "Honda"),
                            ("ford", "Ford"),
                            ("nissan", "Nissan"),
                            ("chevrolet", "Chevrolet"),
                            ("kia", "Kia"),
                            ("hyundai", "Hyundai"),
                            ("volvo", "Volvo"),
                            ("jeep", "Jeep"),
                            ("porsche", "Porsche"),
                            ("lexus", "Lexus"),
                            ("mazda", "Mazda"),
                            ("land_rover", "Land Rover"),
                            ("subaru", "Subaru"),
                            ("fiat", "Fiat"),
                            ("peugeot", "Peugeot"),
                        ],
                        help_text="Car brand, e.g., Toyota, BMW.",
                        max_length=100,
                        verbose_name="Brand",
                    ),
                ),
                (
                    "model",
                    models.CharField(
                        help_text="Car model, e.g., Camry, X5.",
                        max_length=100,
                        verbose_name="Model",
                    ),
                ),
                (
                    "year",
                    models.PositiveIntegerField(
                        help_text="Manufacturing year of the car.", verbose_name="Year"
                    ),
                ),
                (
                    "mileage",
                    models.PositiveIntegerField(
                        help_text="Mileage of the car in kilometers.",
                        verbose_name="Mileage (km)",
                    ),
                ),
                (
                    "fuel_type",
                    models.CharField(
                        choices=[
                            ("diesel", "Diesel"),
                            ("electric", "Electric"),
                            ("petrol", "Petrol"),
                            ("hybrid", "Hybrid"),
                        ],
                        max_length=50,
                        verbose_name="Engine Type",
                    ),
                ),
                (
                    "transmission",
                    models.CharField(
                        choices=[("manual", "Manual"), ("automatic", "Automatic")],
                        max_length=50,
                        verbose_name="Transmission",
                    ),
                ),
                (
                    "listing_type",
                    models.CharField(
                        choices=[("sale", "For Sale"), ("rent", "For Rent")],
                        default="rent",
                        help_text="Whether the item is For sell or Rent.",
                        max_length=200,
                        verbose_name="Listing Type",
                    ),
                ),
                (
                    "condition",
                    models.CharField(
                        choices=[("new", "Brand New"), ("used", "Used")],
                        max_length=200,
                        verbose_name="Condition",
                    ),
                ),
                (
                    "company",
                    models.ForeignKey(
                        blank=True,
                        help_text="The company that owns this listing.",
                        null=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="car_listings",
                        to="account.companyprofile",
                        verbose_name="Company",
                    ),
                ),
                (
                    "individual_owner",
                    models.ForeignKey(
                        blank=True,
                        help_text="The individual person that owns this listing.",
                        null=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="car_listings",
                        to="account.individualownerprofile",
                        verbose_name="Individual Owner",
                    ),
                ),
                ("images", models.ManyToManyField(to="listing.listingimage")),
            ],
            options={
                "verbose_name": "Car Listing",
                "verbose_name_plural": "Car Listings",
                "db_table": "car_listings",
                "constraints": [
                    models.CheckConstraint(
                        condition=models.Q(
                            models.Q(
                                ("individual_owner__isnull", False),
                                ("company__isnull", True),
                            ),
                            models.Q(
                                ("individual_owner__isnull", True),
                                ("company__isnull", False),
                            ),
                            _connector="OR",
                        ),
                        name="car_owner_must_exist",
                    )
                ],
            },
        ),
        migrations.CreateModel(
            name="PropertyListing",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4,
                        editable=False,
                        primary_key=True,
                        serialize=False,
                        verbose_name="Id",
                    ),
                ),
                (
                    "created_at",
                    models.DateTimeField(auto_now_add=True, verbose_name="Created At"),
                ),
                (
                    "updated_at",
                    models.DateTimeField(auto_now=True, verbose_name="Updated_at"),
                ),
                (
                    "title",
                    models.CharField(
                        help_text="Title of the listing, e.g., 'Luxury Suite with Pool View'.",
                        max_length=255,
                        verbose_name="Title",
                    ),
                ),
                (
                    "description",
                    models.TextField(
                        blank=True,
                        help_text="Detailed description of the listing.",
                        verbose_name="Description",
                    ),
                ),
                (
                    "base_price",
                    models.DecimalField(
                        decimal_places=2,
                        help_text="Price of the service/product (if applicable).",
                        max_digits=10,
                        verbose_name="Price",
                    ),
                ),
                (
                    "is_active",
                    models.BooleanField(
                        default=True,
                        help_text="Whether the listing is active and visible.",
                        verbose_name="Is Active",
                    ),
                ),
                (
                    "property_type",
                    models.CharField(
                        choices=[
                            ("apartment", "Apartment"),
                            ("condo", "Condo"),
                            ("villa", "Villa"),
                            ("house", "House"),
                            ("land", "Land"),
                        ],
                        max_length=50,
                        verbose_name="Property Type",
                    ),
                ),
                ("bedrooms", models.PositiveIntegerField(verbose_name="Bedrooms")),
                ("bathrooms", models.PositiveIntegerField(verbose_name="Bathrooms")),
                (
                    "square_meters",
                    models.DecimalField(
                        decimal_places=2, max_digits=10, verbose_name="Square Meters"
                    ),
                ),
                (
                    "is_furnished",
                    models.BooleanField(default=False, verbose_name="Is Furnished"),
                ),
                (
                    "is_for_sale",
                    models.BooleanField(default=False, verbose_name="Is for Sale"),
                ),
                (
                    "address",
                    models.OneToOneField(
                        on_delete=django.db.models.deletion.RESTRICT,
                        related_name="+",
                        to="core.address",
                    ),
                ),
                (
                    "company",
                    models.ForeignKey(
                        blank=True,
                        help_text="The company that owns this listing.",
                        null=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="property_listings",
                        to="account.companyprofile",
                        verbose_name="Company",
                    ),
                ),
                ("images", models.ManyToManyField(to="listing.listingimage")),
                (
                    "individual_owner",
                    models.ForeignKey(
                        blank=True,
                        help_text="The individual person that owns this listing.",
                        null=True,
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="property_listings",
                        to="account.individualownerprofile",
                        verbose_name="Individual Owner",
                    ),
                ),
            ],
            options={
                "verbose_name": "Property Listing",
                "verbose_name_plural": "Property Listings",
                "db_table": "property_listings",
                "constraints": [
                    models.CheckConstraint(
                        condition=models.Q(
                            models.Q(
                                ("individual_owner__isnull", False),
                                ("company__isnull", True),
                            ),
                            models.Q(
                                ("individual_owner__isnull", True),
                                ("company__isnull", False),
                            ),
                            _connector="OR",
                        ),
                        name="property_owner_must_exist",
                    )
                ],
            },
        ),
        migrations.CreateModel(
            name="RoomInventory",
            fields=[
                (
                    "id",
                    models.UUIDField(
                        default=uuid.uuid4,
                        editable=False,
                        primary_key=True,
                        serialize=False,
                        verbose_name="Id",
                    ),
                ),
                (
                    "created_at",
                    models.DateTimeField(auto_now_add=True, verbose_name="Created At"),
                ),
                (
                    "updated_at",
                    models.DateTimeField(auto_now=True, verbose_name="Updated_at"),
                ),
                ("date", models.DateField(db_index=True)),
                (
                    "price",
                    models.DecimalField(
                        blank=True,
                        decimal_places=2,
                        help_text="Price for this date; falls back to room base price if null.",
                        max_digits=10,
                        null=True,
                    ),
                ),
                (
                    "room_listing",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="inventories",
                        to="listing.roomlisting",
                    ),
                ),
            ],
            options={
                "verbose_name": "Room Inventory",
                "verbose_name_plural": "Room Inventories",
                "db_table": "room_inventories",
                "ordering": ["date"],
                "unique_together": {("room_listing", "date")},
            },
        ),
        migrations.AddConstraint(
            model_name="booking",
            constraint=models.CheckConstraint(
                condition=models.Q(
                    models.Q(("room__isnull", False), ("event_space__isnull", True)),
                    models.Q(("room__isnull", True), ("event_space__isnull", False)),
                    _connector="OR",
                ),
                name="booking_exactly_one_target",
            ),
        ),
        migrations.AddConstraint(
            model_name="booking",
            constraint=models.CheckConstraint(
                condition=models.Q(("check_in_date__lt", models.F("check_out_date"))),
                name="booking_valid_dates",
            ),
        ),
    ]
